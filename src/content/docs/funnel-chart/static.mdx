...
<ApiTable>
  ...
  <ApiRow name="backgroundVariant" type="BackgroundVariant">
    Drops `<ChartBackground />` beneath bars while keeping motion-friendly layering internally.
  </ApiRow>
...
<ApiTable>
  <ApiRow name="isClickable" type="boolean" default="false">
    Enables polygon click toggling + synced legend dimming.
  </ApiRow>
...
<ApiTable>
  ...
  <ApiRow name="isClickable" type="boolean" default="false">
    Mirrors `<Stages />` selection toggling.
  </ApiRow>
</ApiTable>

<ApiHeading>Background</ApiHeading>

Pattern backdrop helper mirrored from sibling chart packages. Prefer `backgroundVariant` on `<EvilFunnelChart />` because nested `<Background />` siblings are stripped before funnel bars mount.

<ApiTable>
  <ApiRow name="variant" type="BackgroundVariant" default='"dots"'>
    Which `<ChartBackground />` motif renders when leveraged via the matching root prop workflow.
  </ApiRow>
</ApiTable>
